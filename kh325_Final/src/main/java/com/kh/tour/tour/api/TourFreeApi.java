...
//					System.out.println("\nCurrent Element : " + node.getNodeName());
					if (node.getNodeType() == Node.ELEMENT_NODE) {
						...
						URL url2 = new URL(TourFreeApi.tourFreeUrl(serviceKey, contentId).toString());
...
//							System.out.println(list.toString());
	...
	// 정보페이지 url만들어서 넘김
	public static StringBuffer tourFreeUrl(String serviceKey, int contentId) {
		StringBuffer tourFreeUrl = new StringBuffer();
		tourFreeUrl.append(TOUR_DETAIL_WITH_TOUR_URL);
		tourFreeUrl.append("?" + "serviceKey=" + serviceKey);
