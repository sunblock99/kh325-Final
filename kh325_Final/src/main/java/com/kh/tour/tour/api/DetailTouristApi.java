...
//		practice4.heritageInfo_Url("1", "2", "3");
		DetailTouristApi.callCurrentDetailTouristByXML();
	}

	public static List<DetailTourist> callCurrentDetailTouristByXML() {
		List<DetailTourist> list = new ArrayList<>();
...
//					System.out.println("\nCurrent Element : " + node.getNodeName());
					if (node.getNodeType() == Node.ELEMENT_NODE) {
						...
						if (contentTypeId == 12) {
							URL url2 = new URL(
									DetailTouristApi.touristUrl(serviceKey, contentId, contentTypeId).toString());
...
//						System.out.println("\nCurrent Element2 : " + node.getNodeName());
								if (node2.getNodeType() == Node.ELEMENT_NODE) {
									...
									DetailTourist detailTourist = new DetailTourist(contentId, contentTypeId, heritage1,
											heritage2, heritage3, infoCenter, openDate, restDate, expGuide, expAgeRange,
											accomCount, useSeason, useTime, parking, chkBabyCarriage, chkCreditCard,
											chkPet);
	...
	// 정보페이지 url만들어서 넘김
	public static StringBuffer touristUrl(String serviceKey, int contentId, int contentTypeId) {
		StringBuffer infoUrl = new StringBuffer();
		infoUrl.append(TOUR_TOURLIST_DETAIL_URL);
		infoUrl.append("?" + "serviceKey=" + serviceKey);
		infoUrl.append("&" + TOUR_TOURLIST_DETAIL_EXTRASTRING1);
