			...
			try {
				...
				for (int i = 0; i < nList.getLength(); i++) {
					...
					if (node.getNodeType() == Node.ELEMENT_NODE) {
						...
						URL url2 = new URL(RepeatInfoApi.repeatInfoUrl(serviceKey, contentId, contentTypeId).toString());
						...
						for (int t = 0; t < items.getLength(); t++) {
							...
							NodeList serialNumList = item.getElementsByTagName("serialnum");
							NodeList fldgubunList = item.getElementsByTagName("fldgubun");
							NodeList infoNameList = item.getElementsByTagName("infoname");
							NodeList infoTextList = item.getElementsByTagName("infotext");
					
//						System.out.println("\nCurrent Element2 : " + node.getNodeName());
							for (int k = 0; k < serialNumList.getLength(); k++) {
								int serialNum = Integer.parseInt(((Element) serialNumList.item(k)).getTextContent());
								int fldgubun = Integer.parseInt(((Element) fldgubunList.item(k)).getTextContent());
...
//							System.out.println(list.toString());
	...
	// 정보페이지 url만들어서 넘김
	public static StringBuffer repeatInfoUrl(String serviceKey, int contentId, int contentTypeId) {
		StringBuffer repeatInfoUrl = new StringBuffer();
		repeatInfoUrl.append(TOUR_REPEAT_INFO_URL);
		repeatInfoUrl.append("?" + "serviceKey=" + serviceKey);
		repeatInfoUrl.append("&" + TOUR_REPEAT_INFO_EXTRASTRING1);
		repeatInfoUrl.append("&" + "contentId=" + contentId);
		repeatInfoUrl.append("&" + "contentTypeId=" + contentTypeId);

//			System.out.println(sportsUrl);

		return repeatInfoUrl;
