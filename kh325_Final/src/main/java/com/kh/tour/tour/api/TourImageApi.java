			...
			try {
				...
				for (int i = 0; i < nList.getLength(); i++) {
					...
					if (node.getNodeType() == Node.ELEMENT_NODE) {
						...
						URL url2 = new URL(TourImageApi.tourImageUrl(serviceKey, contentId).toString());
...
//	}
	...
	// 정보페이지 url만들어서 넘김
	public static StringBuffer tourImageUrl(String serviceKey, int contentId) {
		StringBuffer tourImageUrl = new StringBuffer();
		tourImageUrl.append(TOUR_TOURLIST_DETAIL_IMAGE_URL);
		tourImageUrl.append("?" + "serviceKey=" + serviceKey);
