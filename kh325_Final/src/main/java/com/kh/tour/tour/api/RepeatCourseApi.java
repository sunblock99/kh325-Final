...
//						System.out.println(contentTypeId);
						...
						if(contentTypeId == 25) {
						URL url2 = new URL(RepeatCourseApi.repeatCourseUrl(serviceKey, contentId, contentTypeId).toString());
						...
						for (int t = 0; t < items.getLength(); t++) {
							...
							NodeList subNumList = item.getElementsByTagName("subnum");
							NodeList subcontentIdUrlList = item.getElementsByTagName("subcontentid");
							NodeList subNameUrlList = item.getElementsByTagName("subname");
							NodeList subdetailOverviewUrlList = item.getElementsByTagName("subdetailoverview");
							NodeList subdetailImgUrlList = item.getElementsByTagName("subdetailimg");
							NodeList subdetailAltUrlList = item.getElementsByTagName("subdetailalt");
					
							for (int k = 0; k < subNumList.getLength(); k++) {
								int subNum = Integer.parseInt(((Element) subNumList.item(k)).getTextContent());
								int subcontentId = Integer.parseInt(((Element) subcontentIdUrlList.item(k)).getTextContent());
								String subName = ((Element) subNameUrlList.item(k)).getTextContent();
								String subdetailOverview = ((Element) subdetailOverviewUrlList.item(k)).getTextContent();
								String subdetailImg = ((Element) subdetailImgUrlList.item(k)).getTextContent();
								String subdetailAlt = ((Element) subdetailAltUrlList.item(k)).getTextContent();
...
//								System.out.println(list.toString());
	...
	// 정보페이지 url만들어서 넘김
	public static StringBuffer repeatCourseUrl(String serviceKey, int contentId, int contentTypeId) {
		StringBuffer repeatCourseUrl = new StringBuffer();
		repeatCourseUrl.append(TOUR_REPEAT_INFO_URL);
		repeatCourseUrl.append("?" + "serviceKey=" + serviceKey);
